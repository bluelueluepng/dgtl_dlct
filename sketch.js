...
let reverseDictionary = {}; // Dictionary for reverse lookup (symbols → words)
...
function createReverseDictionary() {
  // Flip the dictionary: symbols → words
...
function draw() {
  ...
  textSize(11);
  textAlign(CENTER, BOTTOM);
  let modeText = reverseMode ? 'SYMBOLS → WORDS MODE' : 'WORDS → SYMBOLS MODE';
  text('TYPE → ENTER TO SAVE → ESC TO CLEAR ALL | ' + modeText, width / 2, height - 20);
  pop();
}
